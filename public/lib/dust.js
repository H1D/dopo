...
export function createDust(opts = {}) {
   ...
   */
  function blast(stackEl, kind = "deal") {
    if (tier === 0) return;
    ...
    later(() => {
      ...
      cloud(stage, cx, heavy ? cy - 4 : cy + 10, {
        size: Math.round(w * (heavy ? 0.86 : 0.8)), wide: true, dur: heavy ? 950 : 560,
        dy: -6, s0: heavy ? 0.5 : 0.95, s1: heavy ? 1.3 : 1.7,
        ...
        op: Math.min(1, (heavy ? 0.95 : 0.78) * (tier === 1 ? 1.35 : 1)),
      });
      if (tier >= 2) {
        sim(stage, cx, cy, w, heavy ? 13 : 8);
   ...
   */
  function sim(stage, x, y, w, count = 13) {
    ...
    for (let i = 0; i < count && parts.length < MAX_PARTS; i++) {
      ...
      parts.push({
        ...
        vx: side * (110 + rnd() * 230), vy: -(14 + rnd() * 55),
        size: 70 + rnd() * 90, rot: rnd() * 6.28, vr: (rnd() - 0.5) * 1.6,
